/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { EscrowWhereInput } from "./EscrowWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { EscrowOrderByInput } from "./EscrowOrderByInput";

@ArgsType()
class EscrowFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => EscrowWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => EscrowWhereInput, { nullable: true })
  @Type(() => EscrowWhereInput)
  where?: EscrowWhereInput;

  @ApiProperty({
    required: false,
    type: [EscrowOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [EscrowOrderByInput], { nullable: true })
  @Type(() => EscrowOrderByInput)
  orderBy?: Array<EscrowOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { EscrowFindManyArgs as EscrowFindManyArgs };
