/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { EscrowWhereUniqueInput } from "./EscrowWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { EscrowUpdateInput } from "./EscrowUpdateInput";

@ArgsType()
class UpdateEscrowArgs {
  @ApiProperty({
    required: true,
    type: () => EscrowWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => EscrowWhereUniqueInput)
  @Field(() => EscrowWhereUniqueInput, { nullable: false })
  where!: EscrowWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => EscrowUpdateInput,
  })
  @ValidateNested()
  @Type(() => EscrowUpdateInput)
  @Field(() => EscrowUpdateInput, { nullable: false })
  data!: EscrowUpdateInput;
}

export { UpdateEscrowArgs as UpdateEscrowArgs };
